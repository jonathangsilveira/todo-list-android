...
internal class AddTodoListItemRemoteSyncStrategyTest {
    @get:Rule
    ...
    private val remoteSyncStrategy = AddTodoListItemRemoteSyncStrategy(
    ...
    fun `sync Should throw Exception When payload is null`() = runTest {
        ...
        val entityId = "uuid"
        val payload = null
        val expectedExceptionMessage = "Payload to $entityId must not be null to add TODO list items remotely"
    ...
    fun `sync Should throw Exception When client fails`() = runTest {
        ...
        val entityId = "uuid"
        val payload = "payload"
        val body = TodoListItemRemoteSyncStrategyStubs.addItemBody
        every { decoderMock.decode(payload) } returns body
        coEvery { clientMock.addItem(body) } throws IllegalStateException("Client failed")
    ...
    fun `sync Should throw Exception When local repository fails`() = runTest {
        ...
        val payload = "payload"
        val body = TodoListItemRemoteSyncStrategyStubs.addItemBody
        val response = TodoListItemRemoteSyncStrategyStubs.addItemResponse
        every { decoderMock.decode(payload) } returns body
        coEvery { clientMock.addItem(body) } returns response
